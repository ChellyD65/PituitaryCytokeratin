...
def scaleChannels(img):
    out = np.empty_like(img)
    for chan in np.arange(0,np.shape(img)[len(np.shape(img))-1]):
        x = img[:,:,chan]
        out[:,:,chan] = (x-np.min(x))/np.ptp(x)
    return out

def normChannels(img):
    out = np.empty_like(img)
    for chan in np.arange(0,np.shape(img)[len(np.shape(img))-1]):
        x = img[:,:,chan]
...
def getPSDPolarMean(image):
    return getPolarMean(PowerSpectrumImage(image))

def bandPass(img, lims = [6,60]):
